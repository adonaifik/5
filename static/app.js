...
function setupEventListeners() {
    ...
    if (testSubmissionForm) {
        testSubmissionForm.addEventListener('submit', handleTestSubmission);
    }
...
function displayBloodTypeResult(result) {
    ...
    let html = `
        <div class="blood-type-result">
            ...
            <div class="quiz-actions">
                ...
                <button class="btn btn-info" onclick="testFeedbackPopup()" style="margin-top: 10px;">CLICK HERE TO VERIFY</button>
            </div>
    ...
    console.log('About to show feedback popup for:', result.type, result.confidence);
    setTimeout(() => {
        console.log('Timeout triggered, calling showFeedbackPopup');
...
function showFeedbackPopup(predictedBloodType, confidence) {
    ...
    setTimeout(() => {
        if (!document.querySelector('.feedback-popup-overlay')) {
            alert('Feedback popup failed to appear! Check console for errors.');
        }
    }, 1000);
}
...
function handleFeedbackResponse(isCorrect, predictedBloodType) {
    ...
    if (isCorrect) {
        ...
        // Store feedback first so it's available when selecting actual blood type
        feedbackData.push(feedback);
        localStorage.setItem('bloodTypeFeedback', JSON.stringify(feedbackData));
...
function selectActualBloodType(actualBloodType) {
    ...
    // First, add the feedback entry if it doesn't exist
    if (feedbackData.length === 0 || !feedbackData[feedbackData.length - 1].predictedBloodType) {
        // Get predicted blood type from the result display or store it
        const resultDisplay = document.querySelector('.blood-type-name');
        const predictedType = resultDisplay ? resultDisplay.textContent.replace('Blood Type ', '').trim() : 'Unknown';
        
        const feedback = {
            timestamp: new Date().toISOString(),
            predictedBloodType: predictedType,
            isCorrect: false,
            userAnswers: {...selectedAnswers},
            actualBloodType: actualBloodType
        };
        feedbackData.push(feedback);
    } else {
    ...
    const lastFeedback = feedbackData[feedbackData.length - 1];
    const predictedBloodType = lastFeedback.predictedBloodType;
    
    // Update algorithm weights for incorrect prediction
    updateAlgorithmWeights(predictedBloodType, false, selectedAnswers, actualBloodType);
    
    // Save updated feedback
    localStorage.setItem('bloodTypeFeedback', JSON.stringify(feedbackData));
    
    // Show gratitude and close popup with both predicted and actual
    setTimeout(() => {
        showGratitudeMessage(false, predictedBloodType, actualBloodType);
    }, 500);
}

function showGratitudeMessage(isCorrect, predictedBloodType = null, actualBloodType = null) {
    // Remove existing popup
    const existingPopup = document.querySelector('.feedback-popup-overlay');
    if (existingPopup) {
        existingPopup.remove();
    ...
    if (isCorrect) {
        ...
        // Get predicted blood type from feedback data if not provided
        if (!predictedBloodType && feedbackData.length > 0) {
            const lastFeedback = feedbackData[feedbackData.length - 1];
            predictedBloodType = lastFeedback.predictedBloodType || 'Unknown';
        }
        ...
        message = `
            <div class="feedback-popup">
                ...
                <div class="gratitude-message">
                    <p>We predicted <strong>${predictedBloodType || 'Unknown'}</strong> but you have <strong>${actualBloodType || 'Unknown'}</strong>. This data helps us refine our personality-based predictions.</p>
                    <button class="btn btn-primary" onclick="closeFeedbackPopup()">Continue</button>
                </div>
            </div>
        `;
...
// Test function to manually trigger feedback popup
function testFeedbackPopup() {
    console.log('Testing feedback popup...');
    showFeedbackPopup('A', 85);
}
...
async function handleTestTakerRegistration(e) {
    ...
    try {
        ...
        showMessage(error.message || 'Failed to register test taker. Please try again.', 'error');
    }
}
...
async function handleTestSubmission(e) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/submit_test`, {
            ...
            body: JSON.stringify({
                testPatientId: testData.testPatientId,
        ...
        showMessage(error.message || 'Failed to submit test request. Please try again.', 'error');
    }
}
...
async function handleSearchResults() {
    ...
    try {
        ...
        const testTakerResults = data.results.map(result => ({
            ...
            timestamp: result.processedDate,
            status: result.status
        }));
        
        displayTestTakerResults(testTaker, testTakerResults, data.tests);
        
    } catch (error) {
        console.error('Search error:', error);
        showMessage(error.message || 'Failed to search results. Please try again.', 'error');
    }
}

function displayTestTakerResults(testTaker, testTakerResults, tests = []) {
    const resultsDisplay = document.getElementById('resultsDisplay');
    ...
    let html = `
        <div class="test-taker-results">
            ...
            <div class="test-taker-info">
                ...
                ${testTaker.phone ? `<p><strong>Phone:</strong> ${testTaker.phone}</p>` : ''}
                ${testTaker.email ? `<p><strong>Email:</strong> ${testTaker.email}</p>` : ''}
            </div>
    ...
    if (tests.length === 0) {
        html += '<p>No test requests found for this test taker.</p>';
    } else {
        tests.forEach(test => {
            html += `
                <div class="result-item">
                    <h4>${test.testType.toUpperCase()} Test</h4>
                    ...
                    <p><strong>Submitted:</strong> ${formatDate(test.submissionDate)}</p>
                    ${test.doctorName ? `<p><strong>Doctor:</strong> ${test.doctorName}</p>` : ''}
                    ${test.status === 'pending' ? `<button class="btn btn-primary" onclick="processTest('${test.testId}')">Process Test</button>` : ''}
                </div>
            `;
        });
    }
    ...
    if (testTakerResults.length === 0) {
        ...
        testTakerResults.forEach(result => {
            if (result.type === 'lab_test') {
                html += `
                    <div class="result-item">
                        <h4>${result.testType.toUpperCase()} Test Results</h4>
                        ...
                        <div class="test-analysis">
                            ...
                            <div class="analysis-results">
                                ${Object.entries(result.analysis || {}).map(([param, data]) => `
                                    <div class="analysis-item ${data.status.toLowerCase()}">
                                        <strong>${param.replace(/_/g, ' ').toUpperCase()}:</strong> 
                                        ${data.value} ${data.unit} 
                                        <span class="status-indicator">(${data.status})</span>
                                        <p class="interpretation">${data.interpretation}</p>
                                    </div>
                                `).join('')}
                            </div>
                        </div>
...
async function processTest(testId) {
    try {
        ...
        if (!response.ok) {
            throw new Error(data.error || 'Failed to process test');
        }
        ...
        showMessage(error.message || 'Failed to process test. Please try again.', 'error');
    }
}
